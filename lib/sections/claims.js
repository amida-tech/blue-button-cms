...
function processClaimLine(claimLines) {
    ...
    for (var key in claimLines) {
        ...
        if (ignoreValue(value)) {
            ...
            revenueObj.description = revenueValueArray.length > 1 && revenueValueArray[1].trim();
            ...
            procedureObj.description = procedureValueArray.length > 1 && procedureValueArray[1].trim();
            ...
            modifierObj.description = modifierValueArray.length > 1 && modifierValueArray[1].trim();
            ...
            placeObj.name = placeValueArray.length > 1 && placeValueArray[1].trim();
        } else if (key.indexOf('type of service') >= 0 && value.length >= 0) {
            var typeValueArray = value.split(/-(.+)/);
            typeObj.code = typeValueArray[0].trim();
            typeObj.name = typeValueArray.length > 1 && typeValueArray[1].trim();
